...
const int INITIAL_PARTIES = 5; // Initial number of parties in the world at the start of the simulation.
const int INITIAL_MEMBERS = 3;
const int INITIAL_LOOT = 3;
const int INITIAL_QUESTS = 2;
...
// Display function.
string displayEvent(int eventNum, int & prosperity, int & safety,
    const map < string, array < list < string > , 3 >> & parties); // Function receives event number and returns string based on number.
// Event functions. These can modify parties' and kingdom's variables based on the event number by reference.
string questEvent(int eventNum,
    const string & partyName, array < list < string > , 3 > & party); // This function simulates a quest event.
...
void applyEventEffects(int eventNum,
    const string & partyName, array < list < string > , 3 > & party); // This function uses event numbers to modify parties and kingdom.
string combat(int eventNum,
    const string & partyName, array < list < string > , 3 > & party); // Can remove members and add/delete loot
int randomEvent(mt19937 & gen); // This function generates a random event number to simulate the occurrence of events in our world.
...
int main() {
    ...
    // Load party information from files into vectors for random assignment to parties in the map.
    ...
    for (int i = 0; i < INITIAL_PARTIES; i++) { // Load the initial number of parties into the map with empty lists for members, loot, and quests.
    ...
    // Initialize the first few parties with members, loot, and quests randomly assigned from the vectors we loaded from files.
    ...
    for (int partyCount = 0; partyCount < INITIAL_PARTIES && it != parties.end(); partyCount++, ++it) {
        ...
        for (int count = 0; count < INITIAL_MEMBERS; count++) { // Assign 3 members, 3 loot items, and 2 quests to each party.
            ...
            if (count < INITIAL_QUESTS) { // Only add 2 quests.
    ...
    if (!parties.empty()) { // Check if the map is not empty before trying to display.
    ...
    while (it != parties.end()) { // Loop through the parties in the map and display their information to verify proper loading.
        cout << "Party Name: " << it -> first << endl; // Display the party name (key of the map).
        cout << "Members: ";
        for (const auto & member: it -> second[0]) { // Display the members of the party.
            cout << member << " ";
        }
        cout << endl;
        cout << "Loot: ";
        for (const auto & loot: it -> second[1]) { // Display the loot of the party.
            cout << loot << " ";
        }
        cout << endl;
        cout << "Quests: ";
        for (const auto & quest: it -> second[2]) { // Display the quests of the party.
            cout << quest << " ";
        }
        cout << endl <<
            endl;
    ...
    for (int time = 0; time < TIME_PERIODS; time++) {
        ...
        int kingdomEventNum = randomEvent(gen); // Generate a random event number for the kingdom event to simulate the occurrence of an event in the kingdom.
        ...
        for (auto it = parties.begin(); it != parties.end();) {
            ...
            int partyEvent = randomEvent(gen);
        ...
        if (parties.size() < MAX_PARTIES && rand() % 10 < 4) {
            ...
            if (parties.find(newPartyName) == parties.end()) { // Check if the new party name is not already in use to avoid duplicates.
...
string displayEvent(int eventNum, int & prosperity, int & safety,
    const map < string, array < list < string > , 3 >> & parties) {
...
string combat(int eventNum,
    const string & partyName, array < list < string > , 3 > & party) {
    ...
    if (party[0].empty()) { // Check if the party's member list is empty before trying to remove a member.
...
string questEvent(int eventNum,
    ...
    if (party[0].empty()) { // Check if the map is empty before trying to access it.
...
int randomEvent(mt19937 & gen) {
    //Updated for better randomness.
    uniform_int_distribution < > dis(2, NUM_EVENTS); // Create a uniform distribution for event numbers between 2 and NUM_EVENTS.
    return dis(gen); // Generate and return a random event number from the distribution.
}
// NOTES:
/*
